...
from .cpg_actions import CPGFilteredJointPositionActionCfg
from .cpg_cfg import CPGCfg, FANFAN_POLICY_JOINT_ORDER
from .randomization_profile_loader import load_randomization_profile
...
class FanfanA1CleanRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    def __post_init__(self):
        ...
        randomization_profile = load_randomization_profile()
        motor_random = randomization_profile.get("motor_strength_randomization", {})
        delay_random = randomization_profile.get("action_delay_randomization", {})
        pd_random = randomization_profile.get("pd_gain_randomization", {})
        cpg_random = randomization_profile.get("cpg_limits", {})
        contact_random = randomization_profile.get("contact_randomization", {})

        cpg_cfg = CPGCfg()
        cpg_cfg.mode = "joint_sine"
        cpg_cfg.freq_min = float(cpg_random.get("freq_min_hz", cpg_cfg.freq_min))
        cpg_cfg.freq_max = float(cpg_random.get("freq_max_hz", cpg_cfg.freq_max))
        ...
        self.actions.joint_pos = CPGFilteredJointPositionActionCfg(
            asset_name="robot",
            joint_names=list(FANFAN_POLICY_JOINT_ORDER),
            ...
            sim_motor_delay_steps_range=tuple(delay_random.get("frames", (1, 3))),
            sim_motor_strength_scale_range=tuple(motor_random.get("global_scale", RS01_MOTOR_STRENGTH_SCALE_RANGE)),
            sim_kp=RS01_KP,
            sim_kp_scale_range=tuple(pd_random.get("kp_scale", RS01_STIFFNESS_SCALE_RANGE)),
            sim_kd_scale_range=tuple(pd_random.get("kd_scale", RS01_DAMPING_SCALE_RANGE)),
        )
        ...
        ground_friction = tuple(contact_random.get("ground_friction", (0.6, 1.2)))
        restitution = tuple(contact_random.get("restitution", (0.0, 0.1)))
        self.events.physics_material.params["static_friction_range"] = ground_friction
        self.events.physics_material.params["dynamic_friction_range"] = ground_friction
        self.events.physics_material.params["restitution_range"] = restitution
        ...
        self.rewards.soft_torque_limit = RewTerm(
            func=fanfan_mdp.soft_torque_limit_penalty,
            weight=-0.02,
            params={"soft_torque_limit": RS01_CONTINUOUS_TORQUE, "asset_cfg": SceneEntityCfg("robot")},
        )
        self.rewards.per_leg_residual_balance = RewTerm(
            func=fanfan_mdp.per_leg_residual_balance_penalty,
            weight=-0.10,
        )
        self.rewards.joint_limit_margin = RewTerm(
            func=fanfan_mdp.joint_limit_margin_penalty,
            weight=-0.04,
            params={"margin": 0.08, "asset_cfg": SceneEntityCfg("robot")},
        )
        self.rewards.cpg_phase_contact = RewTerm(
            func=fanfan_mdp.cpg_phase_contact_penalty,
            weight=-0.20,
            params={
                "sensor_cfg": SceneEntityCfg(
                    "contact_forces",
...
@configclass
class FanfanRlCpgRoughEnvCfg(FanfanA1CleanRoughEnvCfg):
    pass


@configclass
class FanfanRlCpgRoughEnvCfg_PLAY(FanfanA1CleanRoughEnvCfg_PLAY):
    pass


@configclass
class FanfanRlCpgRoughEnvCfg_CPG_ONLY(FanfanA1CleanRoughEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
        self.actions.joint_pos.action_mode = "cpg_only"
        self.actions.joint_pos.cpg_cfg.mode = "joint_sine"
        self.actions.joint_pos.cpg_cfg.initial_phase_random = False
        self.actions.joint_pos.cpg_cfg.standing_cmd_threshold = 0.01
